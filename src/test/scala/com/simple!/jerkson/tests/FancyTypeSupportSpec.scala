...
class FancyTypeSupportSpec extends Spec {
  ...
  class `A Joda DateTime` {
    val ts = new DateTime("1883-07-03T00:00:00.000-08:00:00")

    @Test def `Generates a JSON int` {
      generate(ts).must(be("-2729606400000"))
    }

    @Test def `Is parsable from a JSON string` = {
      parse[DateTime]("-2729606400000").compareTo(ts).must(be(0))
    }
  }

  class `A Joda DateTime sequence` {
    val ts = Seq(new DateTime("1883-07-03T00:00:00.000-08:00:00"),
                 new DateTime("1924-06-03T00:00:00.000-08:00:00"))

    @Test def `Generates a JSON int list` {
      generate(ts).must(be("[-2729606400000,-1438358400000]"))
    }

    @Test def `Is parsable from a JSON string` = {
      val dates = parse[Seq[DateTime]]("[-2729606400000, -1438358400000]")
  ...
  class `A case class with a Joda DateTime` {
    val ent = LogEntry(new DateTime("1883-07-03T00:00:00.000-08:00:00"),
                              "birth")

    @Test def `Generates a JSON object` {
      generate(ent).must(be("""{"when":-2729606400000,"message":"birth"}"""))
    }

    @Test def `Is parsable from a JSON object` = {
      val parsedEnt = parse[LogEntry]("""{"when": -2729606400000, "message": "birth"}""")
