...
class JsonGenerationSpec extends Spec {
  ...
  class `A ResizableArray[Int]` {
    def `should generate a JSON array of ints` = {
      generate(mutable.ResizableArray(1, 2, 3)) must beEqualTo("[1,2,3]")
    }
  }

  class `An ArraySeq[Int]` {
    def `should generate a JSON array of ints` = {
      generate(mutable.ArraySeq(1, 2, 3)) must beEqualTo("[1,2,3]")
    }
  }
  ...
  class `A MutableList[Int]` {
    def `should generate a JSON array` = {
      val xs = new mutable.MutableList[Int]
      xs ++= List(1, 2, 3)
  ...
  class `A Queue[Int]` {
    def `should generate a JSON array` = {
      generate(mutable.Queue(1, 2, 3)) must beEqualTo("[1,2,3]")
    }
  }

  class `A ListBuffer[Int]` {
    def `should generate a JSON array` = {
      generate(mutable.ListBuffer(1, 2, 3)) must beEqualTo("[1,2,3]")
    }
  }

  class `An ArrayBuffer[Int]` {
    def `should generate a JSON array` = {
      generate(mutable.ArrayBuffer(1, 2, 3)) must beEqualTo("[1,2,3]")
    }
  }

  class `A Range` {
    def `should generate a JSON array` = {
      generate(Range(1, 4)) must beEqualTo("[1,2,3]")
    }
  }
